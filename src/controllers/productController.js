...
async function uniqueSlug(name, id) {
  ...
  let slug = base, n = 1;
  while (await Product.exists({ slug, ...(id ? { _id: { $ne: id } } : {}) })) slug = `${base}-${n++}`;
  return slug;
}

export async function listPublic(req, res) {
  const { q, category, available, offer, demanded, page = 1, limit = 12, sort = "-createdAt" } = req.query;
  const filter = {};
  ...
  const [items, total] = await Promise.all([
    Product.find(filter).select("+sellingPrice +discountedPrice").populate("category", "name slug").sort(sort).skip(skip).limit(safeLimit).lean(),
    Product.countDocuments(filter)
  ]);
  res.json({ items: items.map(publicProduct), pagination: { page: Number(page), limit: safeLimit, total, pages: Math.ceil(total / safeLimit) } });
}

export async function getPublic(req, res) {
  const p = await Product.findOne({ slug: req.params.slug }).select("+sellingPrice +discountedPrice").populate("category", "name slug").lean();
  if (!p) return res.status(404).json({ message: "Product not found" });
  res.json(publicProduct(p));
}

export async function listAdmin(req, res) {
  const { q, category, available, offer, demanded, page = 1, limit = 20, sort = "-createdAt" } = req.query;
  const filter = {};
  if (q) filter.$or = [{ name: new RegExp(q.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i") }, { sku: new RegExp(q.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i") }];
  if (category) filter.category = category;
  if (available === "true") filter.isAvailable = true;
  if (available === "false") filter.isAvailable = false;
  if (offer === "true") filter.isTodaysOffer = true;
  ...
  const [items, total] = await Promise.all([
    Product.find(filter).select("+purchasePrice +sellingPrice +discountedPrice").populate("category", "name slug").sort(sort).skip((pageNum - 1) * safeLimit).limit(safeLimit).lean(),
    Product.countDocuments(filter)
  ]);
  res.json({ items: items.map(adminProduct), pagination: { page: pageNum, limit: safeLimit, total, pages: Math.ceil(total / safeLimit) } });
}

export async function getAdmin(req, res) {
  const p = await Product.findById(req.params.id).select("+purchasePrice +sellingPrice +discountedPrice").populate("category", "name slug");
  if (!p) return res.status(404).json({ message: "Product not found" });
...
export async function create(req, res) {
  ...
  if (!(await Category.exists({ _id: input.category }))) return res.status(400).json({ message: "Invalid category" });
  const product = new Product({ ...input, slug: await uniqueSlug(input.name) });
  ...
  await product.save();
  res.status(201).json(adminProduct(await Product.findById(product._id).select("+purchasePrice +sellingPrice +discountedPrice").populate("category", "name slug")));
}

export async function update(req, res) {
  const existing = await Product.findById(req.params.id).select("+purchasePrice +sellingPrice +discountedPrice");
  ...
  await existing.save();
  res.json(adminProduct(await Product.findById(existing._id).select("+purchasePrice +sellingPrice +discountedPrice").populate("category", "name slug")));
}
